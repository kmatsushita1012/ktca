...
class ScopeProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        val lensProps = resolver.getSymbolsWithAnnotation(Lens::class.qualifiedName!!)
            .filterIsInstance<KSPropertyDeclaration>()
            .groupBy { it.parentDeclaration?.parentDeclaration as? KSClassDeclaration }
            .filterKeys { it != null }

        val prismActions = resolver.getSymbolsWithAnnotation(Prism::class.qualifiedName!!)
        ...
        for (featureDecl in allFeatureDecls) {
            ...
            OutputStreamWriter(file, Charsets.UTF_8).use { writer ->
                ...
                for (prop in lensGroup) {
                    ...
                    writer.write("""
                        val $featureName.${propName}Lens: Lens<${featureName}.State, $typeName>
                            get() = Lens(
                                get = { it.$propName },
